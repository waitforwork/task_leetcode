...
long long tasks::task_2140(std::vector<std::vector<int> > &questions)
{
    // Получаем количество вопросов
    int n = questions.size();
    // Создаем вектор dp для хранения максимальных значений
    std::vector<long long> dp(n);
    // Инициализируем последний элемент dp значением первого элемента последнего вопроса
    dp[n-1] = questions[n-1][0];
    // Проходим по вопросам в обратном порядке, начиная со второго с конца
    for (int i = n - 2; i >= 0; i--) {
        // Устанавливаем текущее значение dp[i] равным значению первого элемента текущего вопроса
        dp[i] = questions[i][0];
        // Получаем количество вопросов, которые нужно пропустить
        int skip = questions[i][1];
        // Если индекс следующего вопроса, который можно рассмотреть, не выходит за пределы массива
        if (i + skip < n - 1)
            // Добавляем к текущему значению dp[i] значение dp[i + skip + 1]
            dp[i] += dp[i + skip + 1];
        // Обновляем dp[i] с максимальным значением между текущим dp[i] и dp[i + 1]
        dp[i] = std::max(dp[i], dp[i + 1]);
    }
    // Возвращаем максимальное значение, которое можно получить, начиная с первого вопроса
    return dp[0];
}
...
long long tasks::task_2874(std::vector<int> &nums)
{
    int maxi = INT_MIN, diff = 0, n = nums.size();
    long long res = 0;

    for (int i = 0; i < n; i++) {
        maxi = std::max(maxi, nums[i]); // Update the max value seen so far
        if (i >= 2)
            res = std::max(res, (long long)diff * nums[i]); // Compute max triplet value
        if (i >= 1)
            diff = std::max(diff, maxi - nums[i]); // Update max difference
    }

    return res;
}

int tasks::task_1863(std::vector<int> &nums)
{
    int total = 0;
    for (int num : nums) {
...
std::vector<int> tasks::task_368(std::vector<int> &nums)
{
    // Сортируем вектор nums для упрощения поиска делящихся чисел
    sort(nums.begin(), nums.end());

    // Инициализируем вектор dp, где dp[i] будет хранить длину максимальной подпоследовательности, заканчивающейся на nums[i]
    std::vector<int> dp(nums.size(), 1);

    // Инициализируем вектор prev для отслеживания предыдущего индекса в подпоследовательности
    std::vector<int> prev(nums.size(), -1);

    // Переменная для хранения индекса максимальной подпоследовательности
    int maxi = 0;

    // Проходим по всем элементам вектора nums начиная со второго
    for (int i = 1; i < nums.size(); i++) {
        // Сравниваем текущий элемент с предыдущими элементами
        for (int j = 0; j < i; j++) {
            // Проверяем, делится ли nums[i] на nums[j]
            // и обновляем dp[i] и prev[i], если находим более длинную подпоследовательность
            if (nums[i] % nums[j] == 0 && dp[i] < dp[j] + 1) {
                dp[i] = dp[j] + 1; // Обновляем длину подпоследовательности
                prev[i] = j; // Запоминаем индекс предыдущего элемента
            }
        }
        // Обновляем индекс максимальной подпоследовательности, если нашли более длинную
        if (dp[i] > dp[maxi]) maxi = i;
    }

    // Вектор для хранения результата (максимальной подпоследовательности)
    std::vector<int> res;

    // Восстанавливаем подпоследовательность, начиная с индекса maxi
    for (int i = maxi; i >= 0; i = prev[i]) {
        res.push_back(nums[i]); // Добавляем элемент в результат
        if (prev[i] == -1) break; // Если предыдущий элемент отсутствует, выходим из цикла
    }

    // Возвращаем результат в обратном порядке, так как мы восстанавливали его с конца
    std::reverse(res.begin(), res.end());
    return res;
}

bool tasks::task_416(std::vector<int> &nums)
{
    int totalSum = 0;
...
int tasks::task_2176(std::vector<int> &nums, int k)
{
    int pairs=0;
    for (int i = 0 ; i < nums.size() - 1 ; i++) {
...
int tasks::task_2338(int n, int maxValue)
{
    ...
    for (int i = 1; i <= maxValue; ++i)
        for (int bars = 0; bars < std::min(14, n) && cnt[i][bars]; ++bars)
